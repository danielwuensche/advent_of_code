...
def part1(sensors, beacons, y_target):
    ...
    for s in sensors:
        ...
        x_range = [x for x in range(s.bounds[2] + y_distance, s.bounds[3] - y_distance + 1)]
        # x_range = range(s.bounds[2] + y_distance, s.bounds[3] - y_distance + 1)
        # x_coords += [x for x in x_range if x not in x_coords] if x_range else [s.x]
        if x_range:
            x_coords += x_range
        else:
            x_coords.append(s.x)
    ...
    for b in beacons:
        if b[0][1] == y_target and b[0][0] in x_coords:
            x_coords.pop(x_coords.index(b[0][0]))
...
if __name__ == "__main__":
    # print(main("input_example_p1.txt", 10, 20))
